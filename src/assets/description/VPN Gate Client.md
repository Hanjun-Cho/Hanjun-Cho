...
Since my VPN Client does the server availability check on load, the only countries and servers able to be connected are the ones which are at least reachable when the health checks were performed. Within 1 click of the button I can immediately connect to, by default, a Japanese relay server. If it doesn't work then I can change the server and reconnect in 2 clicks.
